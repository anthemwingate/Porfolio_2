...
class Student:
    ...
    def add_grade(self, course, grade):
        ...
        if grade in ['A', 'A-', 'B+', 'B', 'B-', 'C+', 'C', 'C-']:  # Note: records only passing grades
...
class Major:
    ...
    def info(self):
        ...
        """
        return [self.dept, sorted(self.req_courses), sorted(self.elec_courses)]  # Note: each major has only one row
...
def connect_to():
    """ create a database connection to SQLite3 module and corresponding database  """
...
def main():
    ...
    directory_path = f"{dir_abs_path}/EduTrax.py"
    Repository(directory_path)
    query_instructors(connect_to()) # Database access and instructor summary table query
